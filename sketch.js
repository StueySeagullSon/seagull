// Final Optimized Version for Mobile: June 18, 2025
// Stuey's Seagull Son
...
function setup() {
  ...
  // No longer creating graphics objects here to save memory
  
  // Create a reduced number of background elements for performance
  bgLayers.push(new BackgroundLayer(0.2, height / 2, 5, 20, 200, 7)); // Far clouds
  bgLayers.push(new BackgroundLayer(0.5, height / 2 - 50, 10, 30, 150, 5)); // Near clouds
}
...
function runGame() {
  ...
  // Spawn obstacles slightly less frequently
  if (frameCount % 100 === 0) {
  ...
  // Spawn power-ups slightly less frequently
  if (frameCount % 180 === 0) {
...
// --- Direct-drawing functions (more memory efficient) ---
...
function drawChimney(x, y, w, h) {
  push();
  fill(139, 69, 19); rect(x, y, w, h);
  fill(105, 105, 105); rect(x, y, w, 10);
  pop();
}

function drawCrab(x, y, w, h) {
  push();
  translate(x, y);
  noStroke(); fill(255, 69, 0); ellipse(w / 2, h * 0.6, w * 0.8, h * 0.7);
  fill(0); ellipse(w * 0.4, h * 0.4, 5, 5); ellipse(w * 0.6, h * 0.4, 5, 5);
  fill(255, 69, 0); rect(w * 0.1, h * 0.3, 5, 10); rect(w * 0.8, h * 0.3, 5, 10);
  pop();
}

function drawFry(x, y, w, h) {
  push();
  translate(x, y);
  noStroke(); fill(255, 223, 0);
  for (let i = 0; i < 5; i++) { rect(i * 5, 5 + sin(frameCount * 0.1 + i) * 3, 4, h - 10); }
  pop();
}
...
class Player {
  ...
  display() { drawSeagull(this.x, this.y, this.size); } // Calls the direct drawing function
}
...
class PowerUp {
  ...
  update() { this.x -= this.speed; }
  display() { drawFry(this.x, this.y, this.w, this.h); } // Calls the direct drawing function
  offscreen() { return this.x < -this.w; }
}

class BackgroundLayer {
  constructor(speed, y, size, alpha, col, numElements) {
    this.speed = speed; this.y = y; this.elements = [];
  ...
  update() { for (let el of this.elements) { el.x -= this.speed; if (el.x < -el.size) { el.x = width + random(el.size); } } }
